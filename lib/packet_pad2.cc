...
using namespace gr::bar;


class packet_pad2_impl : public packet_pad2 {

public:
packet_pad2_impl(bool debug, bool delay, double delay_sec, unsigned int pad_front, unsigned int pad_tail) : tagged_stream_block("bar_packet_pad2",
...
void set_offset(pmt::pmt_t msg){
	...
	if (psdu_length2 == 1) {
		d_pad_front = (offset[0]-48)*((unsigned int)d_delay_sec); // subtract ascii offset for zero
